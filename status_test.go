		...
		t.Run(mode, func(t *testing.T) {
			t.Run("ListAggregatesByStatus", func(t *testing.T) {
				withStatusStore(t, trimEvents, "status-list", func(
					...
					missingIDs, err := store.ListAggregatesByStatus(
						ctx, "missing",
					)
					assert.NoError(t, err)
				...
				func(t *testing.T) {
					withStatusStore(t,
						trimEvents, "status-remove-mismatch", func(
							...
							err := client.SAdd(
								ctx,
								statusSetKey("status-remove-mismatch", active),
								id.Join(":"),
							).Err()
							assert.NoError(t, err)
							...
							ids, err := store.ListAggregatesByStatus(
								ctx, paused,
							)
							assert.NoError(t, err)

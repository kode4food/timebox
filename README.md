...
Timebox is a small event sourcing library for Go. It gives you an append-only event log, optimistic concurrency, snapshots, append-time indexing, and archive support over pluggable persistence backends.
...
- archive records in `timebox_archive`
...
Archiving moves an aggregate's snapshot and event history into backend-specific archive storage and clears the live records. It is a one-way operation.
